...
/***/ function(module, exports, __webpack_require__) {
	...
	var JaroWinkler = __webpack_require__(3);
	
	var SearchWorker = __webpack_require__(4);
	...
	function computeSearchValues(items, opts) {
		...
		var searchField = opts.searchField;
		var delim = opts.delim;
		var immutable = opts.immutable;
		var removePunctuation = opts.removePunctuation;
		var useWebWorkers = opts.useWebWorkers;
		var searchLowerCase = opts.searchLowerCase;
		var threadCount = opts.threadCount;
		...
		items = typeof items.toArray == "function" ? items.toArray() : items;
		...
		items.forEach(function (item) {
			...
			var curSearchField = Object.prototype.toString.call(searchField) === "[object Array]" ? searchField.reduce(function (acc, f) {
				return acc + item[f] + " ";
			}, "") : item[searchField];
	
			if (removePunctuation) curSearchField = curSearchField.replace(punctuationRE, "");
	 ...
	 */
		if (useWebWorkers && immutable) {
			for (var i = 0; i < threadCount; i++) {
				...
				if (i < 3) slices.push(_searchItems.slice(start, end));else slices.push(_searchItems.slice(start));
			}
		}
	...
	var FuzzySearchResult = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				"li",
				{ className: classes, onClick: this.props.selectItem && this.select, style: this.props.style },
				React.createElement(
					"div",
					{ className: "inline-top", style: { paddingLeft: 4, marginTop: 2 } },
					React.createElement(
						"div",
						null,
	...
	var FuzzySearch = React.createClass({
		...
		componentDidMount: function componentDidMount() {
			...
			if (this.props.initialSelectedID) {
				var selectedItem = this.props.items.filter((function (item) {
					return item[this.props.idField] == this.props.initialSelectedID;
				}).bind(this));
				if (selectedItem.length === 1) this.setState({ selectedItem: selectedItem[0] });
			}
		},
		...
		componentDidUpdate: function componentDidUpdate(prevProps, prevState) {
			if (this.props.items != prevProps.items) {
				...
				if (this.state.active != prevState.active) {
					...
					} else document.removeEventListener("click", this._checkForClose);
				}
	    ...
	    */
					...
					var results = this.state.threadResults[this.state.threadID].reduce(function (acc, res) {
						return acc.concat(res);
					}, []).filter(function (res) {
						return res._score > minScore;
					}).sort(function (a, b) {
		...
		_checkForClose: function _checkForClose(e) {
			if (!containsNode(this.getDOMNode(), e.target)) this.setInactive();
		},
	
		_asyncSearchComplete: function _asyncSearchComplete() {
		...
		_computeData: function _computeData() {
		...
		_createWorkers: function _createWorkers() {
			...
			if (this.state.searchTerm.length) this.search({ target: { value: this.state.searchTerm } });
		},
	
		_closeWorkers: function _closeWorkers() {
			if (this.props.useWebWorkers && this._threads) {
				this._threads.forEach(function (thread) {
					if (thread.worker && thread.worker.terminate) thread.worker.terminate();
				});
			}
		},
	
		_updateScrollTop: function _updateScrollTop() {
			if (this.refs.cont) this.refs.cont.getDOMNode().scrollTop = Math.max(0, this.state.highlightedIdx - 5) * 28;
		},
		...
		render: function render() {
			...
			return React.createElement(
				"span",
				{ className: "fuzzy-search " + this.props.containerClassName },
				React.createElement("input", _extends({}, this.props, {
					...
					ref: "input",
					type: "text",
					value: inactive ? this.state.selectedItem[this.props.nameField] : this.state.searchTerm
				})),
				this.state.active && React.createElement(
					"ul",
					{ className: "fuzzy-results-cont", ref: "cont" },
					items.map(function (result, idx) {
						return React.createElement(this.props.resultsComponent, extend({
		...
		getSearchTerms: function getSearchTerms() {
			return this.state.searchTerm.split(" ").filter(function (term) {
		...
		handleSpecialKeys: function handleSpecialKeys(e) {
			if (e.keyCode == 13) {
				if (this.state.highlightedIdx >= 0) {
					var items = this.getItems(),
					    selectedItem = items[this.state.highlightedIdx];
	
					if (selectedItem) this.selectItem(selectedItem._originalItem);
				...
				var highlightedIdx = Math.min(this.getItems().length - 1, Math.max(0, this.state.highlightedIdx + (e.keyCode == 40 ? 1 : -1)));
	
				this.setState({ highlightedIdx: highlightedIdx }, this._updateScrollTop);
				e.stopPropagation();
				e.preventDefault();
			}
		},
	
		runSearch: function runSearch(searchTerms) {
			...
			for (var i = 0; i < this.state.items.length; i++) {
				...
				for (var j = 0; j < searchTerms.length; j++) {
					...
					for (var k = 0; k < item._searchValues.length; k++) {
						...
						} else if (cache[searchTerm][searchValue]) curDist = cache[searchTerm][searchValue];else curDist = cache[searchTerm][searchValue] = JaroWinkler.get(searchTerm, searchValue);
	    ...
	    */
		...
		setActive: function setActive(e) {
			...
			if (this.props.onFocus) this.props.onFocus(e);else if (e && e.stopPropagation) e.stopPropagation();
		},
		...
		setInactive: function setInactive(e) {
			...
			if (this.props.onBlur) this.props.onBlur(e);else if (e && e.stopPropagation) e.stopPropagation();
		},
	   ...
	   */
		...
		onWorkerMessage: function onWorkerMessage(e) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var jw = {
		...
		get: function get(str1, str2) {
			...
			var matches = 0;
			for (var i = 0; i < 4; i++) {
				if (str1[i] == str2[i]) matches += 1;else break;
			}
	
			return jaroDist + matches * this.weight * (1 - jaroDist);
		},
	
		jaro: function jaro(str1, str2) {
			if (str1 == str2) {
				return 1;
				...
				if (!matches) {
					...
					for (var i = 0; i < str1.length; i++) {
						if (str1Flags[i]) {
							...
							if (str1[i] != str2[j]) transpositions += 1;
						}
					}
		...
		getStringMatches: function getStringMatches(str1, str2) {
			if (str1 == str2) {
				return str1.split().map(function (ch) {
					return { character: ch, isMatch: true, isTransposition: false };
				});
			} else if (!str1.length || !str2.length) {
				return str1.split().map(function (ch) {
					return { character: ch, isMatch: false, isTransposition: false };
				});
			} else {
				var matchWindow = Math.max(0, Math.floor(Math.max(str1.length, str2.length) / 2 - 1)),
				    str1Flags = {},
				...
				if (!matches) {
					...
					for (var i = 0; i < str1.length; i++) {
						if (str1Flags[i]) {
							...
							if (str1[i] != str2[j]) characters.push({ character: str1[i], isMatch: false, isTransposition: true });else characters.push({ character: str1[i], isMatch: true, isTransposition: false });
						} else {
